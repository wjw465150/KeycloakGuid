...
- register-node-at-startup
  ...
  If *true*, then adapter will send registration request to Keycloak. It’s *false* by default and useful only when application is clustered. See [Application Clustering](https://www.keycloak.org/docs/latest/securing_apps/index.html#_applicationclustering) for details
...
4. Start Fuse and install the `keycloak` JAAS realm. The easiest way is to install the `keycloak-jaas` feature, which has the JAAS realm predefined. You can override the feature’s predefined realm by using your own `keycloak` JAAS realm with higher ranking. For details see the [JBoss Fuse documentation](https://access.redhat.com/documentation/en-us/red_hat_jboss_fuse/6.3/html-single/security_guide/#ESBSecureContainer).
